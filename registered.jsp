...
<html>
	...
	<body data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-47" data-genuitec-path="/society/WebRoot/registered.jsp">
		
	<div class="fh5co-loader" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-47" data-genuitec-path="/society/WebRoot/registered.jsp"></div>
		...
		<div class="container">
			...
			<c:if test="${not empty ac}">
				<c:forEach items="${ac}" var="a">
						...
						<h3>加入<span>${a.comunityname}</span></h3>
						<form action="" name="message" method="post">
						<div class="row form-group">
							<div class="col-md-12">
								<!-- <label for="lname">Last Name</label> -->
								<input type="text" id="lname" name="id" class="form-control"value="${a.id}">
							</div>
						</div>
						<div class="row form-group">
							<div class="col-md-6">
								<!-- <label for="fname">First Name</label> -->
								<input type="text" id="fname" name="name" class="form-control" value="${a.name}">
							</div>
							<div class="col-md-6">
								<!-- <label for="lname">Last Name</label> -->
								<input type="text" id="lname" name="position"  class="form-control"value="${a.position}">
							</div>
						</div>
						<div class="row form-group">
							<div class="col-md-6">
								<!-- <label for="fname">First Name</label> -->
								<input type="text" id="fname" name="email" class="form-control" value="${a.email}">
							</div>
							<div class="col-md-6">
								<!-- <label for="lname">Last Name</label> -->
								<input type="text" id="lname" name="tel" class="form-control"value="${a.tel}">
							</div>
						</div>
						<div class="row form-group">
							<div class="col-md-12">
								<!-- <label for="message">Message</label> -->
								<textarea name="content" id="message" cols="30" rows="10" class="form-control">${a.content}</textarea>
							</div>
						</div>
						<div class="form-group">
							<div class="col-md-6">
							<input onclick="document.message.action='down?id=${a.id}&ad=${user.name}&comunityname=${a.comunityname}';
		                    document.message.submit();"
		                     value="反对注册" class="btn btn-primary">
							</div>
							<div class="col-md-6">
							<input onclick="document.message.action='up?id=${a.id}&ad=${user.name}&comunityname=${a.comunityname}';
		                      document.message.submit();" 
		                      value="同意注册" class="btn btn-primary">
							</div>
						</div>
					</form>
				        </div>
